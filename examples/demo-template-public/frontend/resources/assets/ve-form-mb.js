...
veForm.service("VeForm", function() {
	...
	this.applyMask = function(scope, ngModel) { // Call to apply veMask to current string
		...
		angular.forEach(scope.veMask, function(mask, key) { // Remove high numbers
			...
			var test = str[key];
			if(test > mask) {
				str = str.substr(0, key) + str.substr(key + 1);
				change++;
			}
		});
		if(change) { // We have modified the string.
	...
	this.inputFun = function(scope, elem, attr, ngModel) {
		if(attr.autocomplete)
			scope.autocomplete = attr.autocomplete;
		
		scope.veValidator = "";
		scope.type = attr.type || "text";
		
		// If internet explorer, override the type to not validate anything
...
//	if ( attr.index ) console.log("Model: ", scope.data, attr.index, scope.data[attr.objkey],
// scope.data[attr.objkey][attr.index]);
	...
	.directive("myLabel", function() {
		return {
			...
			template: "<label style=\"display: inline-block\">{{label||translations[labelkey||key][data.lang]}}"
				...
				+ "<span ng-if='tip' id='{{tip.id}}' ng-init='loadTip(tip.id,tip.txt)' class='drooltip-icon'></span></span></label>"
		};
	})
	...
	.directive("myTipMessage", function() {
		return {
			...
			template: "<span ng-if=\"tip\" ng-show=\"tips[key]\" class=\"tooltip-box\">"
				...
				+ "<a ng-if=\"k.substr(0,4)=='link'\" href=\"{{v.url}}\" target=\"_blank\">{{v.text}}</a>"
	...
	.directive("myList", function() {
		return {
			...
			template: "<div class=\"data-list\" style=\"{{ddstyle}}\">"
				...
				+ "ng-required=\"required && show\" ng-disabled=\"do(blocked)\" ng-readonly=\"do(readonly)\" "
				+ "ng-click=\"clickFun();$event.stopPropagation()\" style='cursor: pointer' "
	...
	.directive("veInput", function($compile, VeForm) {
		return {
			...
			template: "<div class=\"{{veCols}}\" ng-show=\"do(show)\"> "
				...
				+ "placeholder=\"{{translations[placeholder][data.lang]||placeholder}}\" ng-disabled='do(readonly)' "
				+ "ng-required=\"required && show\" ng-readonly=\"do(readonly)\" style=\"{{ddstyle}}\" "
				+ "ng-class=\"instyle\" ve-validator=\"{{veValidator}}\" autocomplete=\"{{autocomplete}}\"/> "
	...
	.directive("veNumber", function($compile, VeForm) {
		return { // Same as input, but for numbers with spin buttons
			...
			template: "<div class=\"{{veCols}}\" ng-show=\"do(show)\">"
				...
				+ "placeholder=\"{{placeholder||translations[placekey][data.lang]}}\" ng-disabled='do(readonly)' "
				+ "ng-required=\"required && show\" ng-readonly=\"do(readonly)\" style=\"{{ddstyle}}\" "
	...
	.directive("veTextarea", function($compile, VeForm) {
		return {
			...
			template: "<div class=\"{{veCols}}\" ng-show=\"do(show)\"><my-label ng-if=\"!nolabel\"></my-label>"
				...
				+ "placeholder=\"{{translations[placeholder][data.lang]||placeholder}}\" ng-disabled='do(readonly)' "
				+ "ng-required=\"required && show\" ng-readonly=\"do(readonly)\" "
	...
	.directive("veCheckbox", function($compile, $interpolate, VeForm) {
		return {
			...
			template: "<div class=\"{{veCols}}\" ng-show=\"do(show)\">"
				...
				+ "ng-required=\"required && show && isSelected()==0\" ng-click=\"do(item.click)\" ng-disabled=\"do(blocked)\"/>"
			...
			link: function(scope, elem, attr, ctrl, trans) {
				...
				scope.isSelected = function() {
					var cnt = 0;
					angular.forEach(scope.model[scope.key], function(item) {
						if(item) cnt++;
					});
					return cnt;
				};
	...
	.directive("veRadio", function($compile, $interpolate, VeForm) {
		return {
			...
			template: "<div class=\"{{veCols}}\" ng-show=\"do(show)\">"
				...
				+ "ng-click=\"model[key]=i;do(item.click)\" ng-required=\"required && show\" ng-disabled=\"do(blocked)\" />"
