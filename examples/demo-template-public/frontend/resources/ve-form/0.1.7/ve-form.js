...
veForm.service('VeForm', function () {
	this.menus = { test: false }; // Used for all drop-down menus.
	this.tips = {};  // Tracks all tooltips
	this.flags = { // List of languages and locales associated with the flags defined in css.
		"en": {country: "UK", name:"English", locale:"en_GB"},
		"nb": {country: "Norge", name:"Norsk", locale:"nb_NO"},
		"sv": {country: "Sverige", name:"Svensk", locale:"sv_SE"},
		"da": {country: "Danmark", name:"Dansk", locale:"da_DK"},
		"fi": {country: "Suomi", name:"Suomi", locale:"fi_FI"},
		"fr": {country: "France", name:"Français", locale:"fr_FR"},
		"de": {country: "Deutchland", name:"Deutch", locale:"de_DE"},
		"nl": {country: "Holland", name:"Nederlands", locale:"nl_NL"},
		"es": {country: "España", name:"Español", locale:"es_ES"},
		"pl": {country: "Polska", name:"Polskie", locale:"pl_PL"}
	};
	this.country = function(lang) { return this.flags[lang||"en"].country }
	this.locale = function(lang) { return this.flags[lang||"en"].locale }
	this.language = function(lang) { return this.flags[lang||"en"].name }
	...
	this.fix_tr = function(t) { // Translations array->object
		var n_keys = t.length-1;
		var n_lang = t[0].length-1;
		var ret = {};
		for ( var i=1; i<=n_keys; i++ ) {
			var key = t[i][0];
			ret[key] = {};
			for ( var l=1; l<=n_lang; l++ ) {
				var lang = t[0][l];
				ret[key][lang] = t[i][l];
			}
		}
	...
	this.inputFun = function(scope, elem, attr, ngModel) {
		...
		if ( attr.type == 'dob' ) {
			...
			scope.placeholder = scope.placeholder || "YYYY-MM-DD";
			scope.veValidator = "0123456789";
			scope.maxLength = 10;
		}
		if ( attr.type == 'date' ) {
			...
			scope.placeholder = scope.placeholder || "DD-MM-YYYY";
			scope.veValidator = "0123456789";
			scope.maxLength = 10;
		}
		if ( attr.type == 'ssn-no' ) {
			...
			scope.veValidator = "0123456789";
			scope.placeholder = scope.placeholder || "DDMMÅÅ NNNNN";
			scope.maxLength = 12;
		}
		if ( attr.type == 'ssn-se' ) {
			...
			scope.veValidator = "0123456789";
			scope.placeholder = scope.placeholder || "ÅÅÅÅMMDD-NNNN";
			scope.maxLength = 13;
		}
		if ( attr.type == 'ssn-dk' ) {
			scope.pattern = /^[0-3][0-9][0-1][0-9][0-9][0-9]\-[0-9]{4}$/;
			scope.type = 'text';
			scope.veMask = "391999-9999"
			scope.veValidator = "0123456789";
			scope.placeholder = scope.placeholder || "DDMMÅÅ-NNNN";
			scope.maxLength = 13;
		}
	...
	this.linkFun = function(scope, elem, attr, type, ngModel) {
		...
		if ( attr.object ) {
			var objectPath = attr.object.split('.');
			scope.model = objectPath.reduce(function (obj, key) {
				return obj[key];
			}, scope);
		}
		if ( attr.objkey ) {
...
//	if ( attr.index ) console.log("Model: ", scope.data, attr.index, scope.data[attr.objkey], scope.data[attr.objkey][attr.index]);
		...
		if ( attr.break || attr.break == "" ) scope.break = true;
		...
		scope.tips = attr.tips ? attr.tips : this.tips;
		if ( attr.buttons ) scope.buttons = JSON.parse(attr.buttons);
		if ( attr.list ) scope.list = scope.lists[attr.list] ? scope.lists[attr.list] : ( scope[attr.list] ? scope[attr.list] : JSON.parse(attr.list));
		if ( attr.details ) scope.details = scope.lists[attr.details] ? scope.lists[attr.details] : JSON.parse(attr.details);
		...
		if ( attr.redtext ) scope.redtext = attr.redtext;
		...
		if ( scope.role ) scope.required = attr.required == "true" ? "role==\"" + attr.role +"\"" : "";
		if ( attr.restricted === "" && attr.role )  attr.readonly = "role!=\"" + attr.role +"\"";
		scope.readonly = attr.readonly;
		scope.show = attr.show ? attr.show : "true";
		scope.filter = attr.filter ? attr.filter : "true";
		if ( attr.cols || attr.cols == "" ) {
			scope.cols = attr.cols;
			scope.veCols = this.getCols(attr.cols);
		}
		scope.lastyle = scope.ddstyle = "";
		if ( attr.large || attr.large == "" ){
			scope.lastyle += "font-size: 2rem;";
		}
		...
		if ( type == 'dropdown' || type == 'radio' || type == 'checkbox' ||
			   type == 'input' || type == 'list' || type == 'language' ||
			   type == 'flags' || type == 'query' ) {
			if ( type == 'dropdown' && attr.noday=="" ) scope.noday = true;
			scope.menus = attr.menus ? attr.menus : this.menus;
		...
		if ( type == 'query' ) {
			...
			scope.checkbox = angular.isUndefined(attr.checkbox) ? false : true;
		}
		if ( type == 'header' ) {
			...
			var validate = function(key) {
				...
				if ( scope.pattern && !scope.required && val == "" ) {
					...
					if ( valid && scope.veMin )
						valid = ( parseFloat(ngModel.$viewValue) >= parseFloat(scope.veMin) )
					if ( valid && scope.veMax )
						valid = ( parseFloat(ngModel.$viewValue) <= parseFloat(scope.veMax) )
...
.directive( 'myLabel', function() {
	return {
			...
			scope: false,
			replace: true,
			template: '<label ng-model="labelkey" style="display: inline-block">{{label||translations[labelkey||key][data.lang]||model[labelkey]}}'
...
.directive( 'myTip', function() {
	return {
			...
			template: '<span ng-if="tip" class="tooltip">'
							+ '<span class="tooltip-message" ng-click="tips[key]=!tips[key]"></span></span>'
	}
})
.directive( 'myTipMessage', function() {
	return {
			...
			scope: false,
			replace: true,
			template: '<span ng-if="tip" ng-show="tips[key]" class="tooltip-box">'
...
.directive( 'myList', function() {
	return {
			...
			template: '<div class="data-list" style="{{ddstyle}}">'
							...
							+ '<li ng-repeat="(k,v) in items track by $index" tabindex="1" ng-if="filter && v[filter]==data[filter]" '
...
.directive( 'veNumber', function ( $compile, VeForm ) {
	return { // Same as input, but for numbers with spin buttons
		restrict: 'AE',
		scope: true,
		replace: true,
		template: '<div class="{{veCols}}" ng-show="do(show)">'
						+ '<my-label ng-if="!nolabel" style="{{lastyle}}"></my-label>'
						+ '<span ng-if="tip" class="tooltip">&nbsp;'
						+ '<span class="tooltip-message" ng-click="tips[key]=!tips[key]"></span></span>'
						+ '<input type="number" ng-model="model[key]" ve-change="{{veChange}}" '
						+ 'placeholder="{{placeholder||translations[placekey][data.lang]}}" '
						+ 'ng-required="do(required) && do(show)" ng-readonly="do(readonly)" style="{{ddstyle}}" '
						+ 'ng-class="instyle" ve-validator="{{veValidator}}" min={{veMin}} max={{veMax}}/>'
						+ '<span ng-if="tip" ng-show="tips[key]" class="tooltip-box">{{tip.text}}</span>'
						+ '</div>',
		link: function(scope,elem,attr) {
			attr.type = "int-minmax";
			VeForm.linkFun(scope,elem,attr,'input');
			scope.do = function (it) { return scope.$eval( it ); }
		}
	}
...
.directive( 'veTextarea', function ( $compile, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}}" ng-show="do(show)"><my-label ng-if="!nolabel"></my-label>'
						...
						+ 'placeholder="{{translations[placeholder][data.lang]||placeholder}}" '
						+ 'ng-required="do(required) && do(show)"  ng-readonly="do(readonly)" '
						+ 've-validator="{{veValidator}}"></textarea></div>',
		link: function(s,e,a) {
...
.directive( 'veDropdown', function ( $compile, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}}" ng-show="do(show)">'
						...
						+ '<my-list></my-list>'
						+ '<my-tip-message></my-tip-message>'
						+ '<div ng-repeat="(item,value) in alerts"><span ng-if="item==model[key]" class="alert alert-info">{{translations[value][data.lang]||value}}</span>'
						+ '</div></div>',
		link: function(s,e,a) {
...
.directive( 'veDateDrop', function ( $compile, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}}" ng-show="do(show)">'
						...
						+ '<div class="data-list" style="display: inline-block; width: 25%" ng-if="!noday">'
						+ '<input type="text" ng-model="model[key].day" ng-readonly="true" ng-required="do(required)" '
						+ 'ng-click="menus[dkey]=true; $event.stopPropagation()" placeholder="{{translations.DD[data.lang]}}" />'
						...
						+ '<input type="text" ng-model="model[key].month" ng-readonly="true" ng-required="do(required)" '
						+ 'ng-click="menus[mkey]=true; $event.stopPropagation()" placeholder="{{translations.MM[data.lang]}}" />'
						+ '<ul ng-show="menus[mkey]" menu-closer="menus[mkey]">'
						...
						+ '<input type="text" ng-model="model[key].year" ng-readonly="true" ng-required="do(required)" '
						+ 'ng-click="menus[ykey]=true; $event.stopPropagation()" placeholder="{{translations.YYYY[data.lang]}}"/>'
...
.directive( 'veListSelector', function ( $compile, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}} ng-show="do(show)">'
						...
						+ '<input type="text" ng-model="model[key]" '
						+ 'placeholder="{{translations[placeholder][data.lang]||placeholder}}" '
						+ 'ng-required="do(required)" ng-readonly="do(readonly)" style="{{ddstyle}}" '
...
.directive( 'veCounter', function ( $compile, $interpolate, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}} ng-show="do(show)">'
			...
			+ '<small ng-if="redtext" style="color: red"><span ng-if="inline">&nbsp;&nbsp;</span><br ng-if="!inline">{{translations[redtext][data.lang]||redtext}}</small>'
			+ '<small ng-if="small"><span ng-if="inline">&nbsp;&nbsp;</span><br ng-if="!inline">{{translations[small][data.lang]||small}}</small>'
			+ '</div>',
...
.directive( 'veQuery', function ( $compile, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}}" ng-show="do(show)">'
						...
						+ '<br ng-if="break" />'
						+ '<div ng-transclude></div>'
						+ '<div ng-if="!checkbox" ng-repeat="item in [\'yes\', \'no\']" style="display: inline-block">'
						...
						+ '</div>'
						+ '<div ng-if="checkbox" style="display: inline-block">'
						+ '<input type="checkbox" name="{{idkey}}" id="{{key}}-1" ng-model="model[key]" ng-checked="model[key]" />'
						+ '<label for="{{key}}-1">{{text||translations[item][data.lang]}}</label>'
						+ '<small ng-if="small&&model[key]">{{small}}</small>'
						+ '</div>'
						+ '<div ng-if="checkbox?model[key]:model[key]==(negative?\'no\':\'yes\')">'
						...
						+ '<div ng-if="item.input==\'checkbox\'" style="display: inline-block">'
						+ '<input type="checkbox" name="{{idkey}}" id="{{key}}-{{item}}" ng-model="model[conditional][i]" />'
						+ '<label for="{{key}}-{{item}}">{{item.text||translations[item][data.lang]}}</label>'
						+ '<div ng-if="model[conditional][i]&&item.items" ng-repeat="(j,val) in item.items" style="display: inline-block">'
						+ '<input type="text" ng-model="model[conditional][j]" ng-required="true" placeholder="{{translations[val.input][data.lang]||val.input}}" style={{val.style}} />'
						+ '</div></div>'
...
.directive( 'veMultiQuery', function ( $compile, VeForm ) {
	return {
		restrict: 'AE',
		scope: true,
		replace: true,
		...
		template: '<div class="{{veCols}}" ng-show="do(show)">'
						...
						+ 'ng-click="model[key]=item" ng-required="true"/>'
						+ '<label for="{{key}}-{{item}}">{{text||translations[item][data.lang]}}'
						+ '</div><div ng-if="model[key]==(negative?\'no\':\'yes\')"><div ng-repeat="con in model[conditional]">'
						+ '<div ng-repeat="(i,item) in items" style="width: 100%; padding: 2px 0 2px 0;" >'
						+ '<input ng-if="item.input==\'text\'||!item.input" type="text" ng-model="con[i]" ng-required="true" placeholder="{{translations[item.text][data.lang]||item.text}}" />'
						+ '<div ng-if="item.input==\'radio\'" style="padding: 6px 2px 4px 2px; display: inline-block">{{translations[item.text][data.lang]||item.text}}&nbsp;&nbsp;</div>'
						+ '<div ng-if="item.input==\'radio\'" ng-repeat="(j,val) in item.items" style="display: inline-block">'
						+ '<input type="radio" name="{{idkey}}-{{i}}" id="{{idkey}}-{{i}}-{{j}}" ng-model="con[i]" ng-checked="con[i]==j"'
						+ 'ng-click="con[i]=j" ng-required="true" />'
						+ '<label for="{{idkey}}-{{i}}-{{j}}">{{translations[j][data.lang]||j}}</label>'
						+ '<input ng-if="val.input&&model[conditional][i]==j" type="text" ng-model="con[val.input]" ng-required="true" placeholder="{{translations[val.input][data.lang]||val.input}}" style={{val.style}} />'
						+ '</div>'
						+ '</div><hr></div>'
						+ '<div ng-repeat="(key,item) in buttons" style="display: inline-block; margin-right: 1rem; margin-top: 0.5rem">'
						+ '<input type="button" ng-disabled="do(item.disabled)" ng-click="do(item.click)"'
						+ ' value="{{item.value||translations[key][data.lang]}}" style="padding: 0.6rem; margin: 0"/>'
						+ '</div>'
						+ '</div></div>',
		link: function(s,e,a) {
			VeForm.linkFun(s,e,a,'query');
			s.buttons = {
				"add":{"click":"model[conditional].push({})","disabled":"model[conditional].length>=10"},
				"remove":{"click":"model[conditional].pop()","disabled":"model[conditional].length<=1"}
			};
			s.do = function (it) { return s.$eval( it ); }
		}
	}
...
.directive( 'veCheckbox', function ( $compile, $interpolate, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}}" ng-show="do(show)">'
			...
			+ 'ng-required="do(required) && do(show) && isSelected()==0" ng-click="do(item.click)" ng-disabled="do(item.blocked)"/>'
			...
			+ '<span ng-if="item.items" ng-repeat="it in item.items" >'
			+ '<input ng-if="it.input" type="{{it.input}}" ng-model="model[it.key]" placeholder="{{translations[it.placeholder][data.lang]||it.placeholder}}"'
			+ 'style="{{it.style}}" ng-required="model[key][i]" ng-disabled="!model[key][i]" /></span>'
			+ '<span ng-if="item.unit">&nbsp;{{item.unit}}</span>'
			+ '<div ng-if="item.small" style="margin: -0.5rem 0 0 0">'
			+ '<small>{{translations[item.small][data.lang]||item.small}}</small></div>'
			...
			+ '<span ng-if="k.substr(0,3)==\'url\'"><a href="{{v.link}}" target="_new">{{translations[v.text][data.lang]||v.text}}</a></span>'
			...
			+ ' value="{{item.value||translations[key][data.lang]}}" style="padding: 0.6rem; margin: 0"/>'
			+ '</span>'
			+ '<div ng-repeat="(item,value) in alerts"><span ng-if="model[key][item]" class="alert alert-info">{{translations[value][data.lang]||value}}</span></div>'
			+ '</div>',
		link: function(scope,elem,attr,ctrl,trans) {
...
.directive( 'veRadio', function ( $compile, $interpolate, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}}" ng-show="do(show)">'
			...
			+ 'ng-click="model[key]=i;do(item.click)" ng-required="do(required) && do(show)" ng-disabled="do(blocked)" ve-change="{{veChange}}" />'
			...
			+ '<span ng-if="item.after">{{translations[item.after][data.lang]||item.after}}&nbsp;&nbsp;</span>'
...
.directive( 'veItemize', function ( $compile, $interpolate, VeForm ) {
	return {
		restrict: 'AE',
		scope: true,
		replace: true,
		template: '<div class="{{veCols}} ng-show="do(show)">'
			+ '<my-label ng-if="!nolabel" style="{{lastyle}}"></my-label>'
			+ '<ul>'
			+ '<li ng-repeat="item in list">{{details[item].text||item}}&nbsp;&nbsp;'
			+ '<span ng-if="details[item].tip" class="tooltip-message" ng-click="do(tips[item]=!tips[item])"></span>'
			+ '<i ng-if="details[item].price">&nbsp;{{details[item].price}}</i>'
			+ '<span class="tooltip-box" ng-if="tips[item]">{{details[item].tip}}</span>'
			+ '</li></ul>{{list}}</div>',
		link: function(scope,elem,attr,ctrl) {
			VeForm.linkFun(scope,elem,attr,'itemize');
			scope.do = function (it) { return scope.$eval( it ); }
		}
	}
})
...
.directive( 'veBusy', function ( $compile, VeForm ) {
	return {
		...
		template: '<div ng-show="do(show)" style="text-align: center">'
			+ '<img src="../common/assets/loader.gif" /><h4>'
...
.directive( 'veFlags', function ( $compile, VeForm ) {
	return {
		restrict: 'AE',
		scope: true,
		replace: true,
		template: '<div ng-show="do(show)"><ul class="languages">'
			+ '<li ng-repeat="l in languages">'
			+ '<a ng-click="data.lang=l"><i class="flag {{flag(l)}}"></i></a>'
			+ '</li></ul></div>',
		link: function(scope, elem, attr) {
			VeForm.linkFun(scope,elem,attr,'flags');
			scope.do = function (it) { return scope.$eval( it ); }
			scope.flag = function (l) { return VeForm.locale(l); }
		}
	}
})
