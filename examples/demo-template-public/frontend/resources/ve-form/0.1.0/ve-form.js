...
//	if ( attr.index ) console.log("Model: ", scope.data, attr.index, scope.data[attr.objkey], scope.data[attr.objkey][attr.index]);
		...
		if ( type == 'conditional-input' ) this.conditionalFun(scope, elem, attr);
		if ( type == 'header' ) {
...
.directive( 'myLabel', function() {
	return {
			...
			scope: false,
			replace: true,
			template: '<span><label style="display: inline-block">{{label||translations[labelkey||key][data.lang]}}</label>'
							+ '<span ng-if="tip">&nbsp;&nbsp;<my-tip></my-tip></span></span>'
	}
})
...
.directive( 'veInput', function ( $compile, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}}" ng-show="do(show)">'
						...
						+ 'placeholder="{{translations[placeholder][data.lang]||placeholder}}" '
						+ 'ng-required="required && do(show)" ng-readonly="do(readonly)" style="{{ddstyle}}" '
...
.directive( 'veTextarea', function ( $compile, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}}" ng-show="do(show)"><my-label ng-if="!nolabel"></my-label>'
						...
						+ '<textarea type="text" ng-model="model[key]" ve-change="{{veChange}}" '
						+ 'placeholder="{{translations[placeholder][data.lang]||placeholder}}" '
						+ 'ng-required="required"></textarea></div>',
		link: function(s,e,a) {
...
.directive( 'veLabel', function ( $compile, $interpolate, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}}" ng-show="do(show)">'
			...
			+ '<label ng-if="labelkey||translations[key]" style="margin: 0;{{lastyle}}">{{translations[labelkey][data.lang]||translations[key][data.lang]}}&nbsp;</label>'
			+ '<my-tip></my-tip>'
			...
			+ '<small ng-if="small"><span ng-if="inline">&nbsp;&nbsp;</span>{{translations[small][data.lang]||small}}</small>'
...
.directive( 'veConditionalInput', function ( $compile, VeForm ) {
	return {
		restrict: 'AE',
		scope: true,
		replace: true,
		template: '<div class="{{veCols}}" ng-show="do(show)><my-label></my-label>'
						+ '<div ng-repeat="item in [\'yes\', \'no\']" style="display: inline-block">'
						+ '<input type="radio" name="{{key}}" id="{{key}}-{{item}}" ng-model="model[key]" ng-checked="model[key]==item"'
						+ 'ng-click="model[key]=item" ng-required="true"/>'
						+ '<label for="{{key}}-{{item}}">{{text||translations[item][data.lang]}}'
						+ '</div><div ng-if="model[key]==\'yes\'" style="display: inline-block">'
						+ '<div ng-repeat="item in items track by $index" style="display: inline-block">'
						+ '<input type="text" ng-model="model[conditional]" ng-required="true" placeholder="{{text||translations[item][data.lang]}}" />'
						+ '</div></div></div>',
		link: function(s,e,a) {
			VeForm.linkFun(s,e,a,'conditional-input');
			scope.do = function (it) { return scope.$eval( it ); }
		}
	}
})
.directive( 'veCheckbox', function ( $compile, $interpolate, VeForm ) {
	return {
		...
		replace: true,
		transclude: true,
		template: '<div class="{{veCols}}" ng-show="do(show)" ng-init="model[key]=[]">'
			+ '<div ng-transclude></div>'
			...
			+ '<label for="{{idkey}}-{{i}}">{{translations[item.key][data.lang]||translations[item.text][data.lang]||item.text}}</label>'
...
.directive( 'veRadio', function ( $compile, $interpolate, VeForm ) {
	return {
		...
		template: '<div class="{{veCols}}" ng-show="do(show)">'
			...
			+ '<span class="tooltip-message" ng-click="tips[i]=!tips[i]" style="margin-left: -2rem"></span>&nbsp;&nbsp;</span>'
			+ '<input ng-if="item.input" type="{{item.input}}" ng-model="model[item.key]" placeholder="{{item.placeholder}}"'
			+ 'style="{{item.style}}" ng-required="do(show) && model[key]==i" ng-disabled="model[key]!=i" />'
			+ '<span ng-if="item.tip" ng-show="tips[i]" class="tooltip-box">'
			+ '<span ng-repeat="(k,v) in item.tip track by $index">'
			+ '<span ng-if="k.substr(0,4)==\'text\'">{{translations[v][data.lang]||v}}</span>'
			...
			+ '</div>'
			+ '<my-tip-message></my-tip-message>'
			+ '<div><small ng-if="small">{{translations[small][data.lang]||small}}</small><div>'
