...
brReg.service('BrReg', function($q, $http, VeForm){
	...
	this.update = function(scope,d) {
		...
		scope.model.orgForm = d.organisasjonsform;
	};
});
...
.directive('brregNr', function($parse, $timeout, BrReg, VeForm) {
	return {
		...
		scope: true,
		replace: true,
		template: '<div class="{{veCols}}" ng-show="do(show)"><label>{{label||translations[labelkey][data.lang]||translations[key][data.lang]}}</label>'
			+ '<input type="text" ng-model="model[key]" placeholder="{{translations[placeholder][data.lang]||placeholder}}" ve-change="{{veChange}}" '
			+ 'ng-required="do(required)" ng-readonly="do(readonly)"'
...
.directive('brregName', function(BrReg, VeForm) {
	return {
		...
		template: '<div class="{{veCols}}" ng-show="do(show)"><label>{{label||translations[labelkey][data.lang]||translations[key][data.lang]}}</label>'
			+ '<div class="data-list">'
			+ '<input type="text" ng-model="model[key]" placeholder="{{translations[placeholder][data.lang]||placeholder}}" ve-change="{{veChange}}" '
			+ 'ng-change="menus[mkey]=true; $event.stopPropagation()" ng-click="menus[mkey]=!menus[mkey]"'
			+ 'ng-required="do(required)" ng-readonly="do(readonly)" ve-validator="{{veValidator}}" get-brreg-name />'
			+ '<ul ng-show="menus[mkey]" menu-closer="menus[mkey]">'
			+ '<li ng-repeat="(i,value) in lists.BrRegDatalist track by $index"'
			+ ' ng-click="model[key]=i; menus[mkey]=!menus[mkey]">{{i}}</li>'
			+ '</ul></div></div>',
		link: function(s,e,a) {
			a.type = "brreg-name"
			VeForm.linkFun(s,e,a,'input');
...
.directive('brregStatusBox', function(BrReg, VeForm) {
	return {
		restrict: 'E',
		scope: true,
		replace: true,
		template: '<div class="flex-columns" style="width: 100%" ng-show="do(show)"><div class="col col-2">'
